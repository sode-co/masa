...
import com.devlogs.masa_backend.meeting.GetAllMeetingsByTitleUseCase;
...
@WebServlet(name = "getmeetingbytitle", urlPatterns = {"/api/meeting-management/meetings/title/*"})
public class GetAllMeetingsByTitleServlet extends BaseHttpServlet {
    @Inject
    protected GetAllMeetingsByTitleUseCase getAllMeetingsByTitleUseCase;
    @Inject
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        GetAllMeetingsByTitleUseCase.Result result = getAllMeetingsByTitleUseCase.executes(title);
        System.out.println(result.toString());
        if (result instanceof GetAllMeetingsByTitleUseCase.Result.ConnectionError) {
            getRequestComponent().getResponseHelper().responseMessage(500, "Connection to db error");
        } else if ( result instanceof GetAllMeetingsByTitleUseCase.Result.Success) {
            ...
            for (MeetingEntity meeting : ((GetAllMeetingsByTitleUseCase.Result.Success) result).meetings) {
